...
def calculateD(numbers):
    ...
    for k in range(len(numbers)):
        if k != 0:
            d.append(math.ceil(numbers[k]-numbers[0]))
    return d
...
def processStego(coverImage, secretBits):
    lenI = math.ceil(len(coverImage)/3)
    lenJ = math.ceil(len(coverImage[0])/3)
        ...
        for j in range(lenJ):
            ...
            for k in range(len(b)):
                ...
                if len(secretBits) >= stop:
                    secretBits = secretBits[0: start:] + secretBits[stop::]
                if len(secretBits) == 0:
...
coverImage = makeInterpolation(gray)

# 3) Create secret message
# secretValue = 19 22 5
# secretBit = to_bin(secretValue)
secretBit = '1001101011000101'

stegoImage = processStego(coverImage, secretBit)
